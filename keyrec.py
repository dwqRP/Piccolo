...
def get_const_seq():
    for i in range(num_rounds):
        stream = 0x6547a98b ^ cat(cat(cat(cat(cat(cat(i + 1, 0, 5), i + 1, 5), 0, 2), i + 1, 5), 0, 5), i + 1, 5)
...
def RP(x0, x1, x2, x3):
    return cat(lhalf(x1), rhalf(x3)), cat(lhalf(x2), rhalf(x0)), cat(lhalf(x3), rhalf(x1)), cat(lhalf(x0), rhalf(x2))
...
class Piccolo:
    ...
    def init_key(self, master_key):
        ...
        for i in range(num_rounds << 1):
            if (i + 2) % 8 == 0:
                k[0], k[1], k[2], k[3], k[4], k[5], k[6], k[7] = k[2], k[1], k[6], k[7], k[0], k[3], k[4], k[5]
                # print(i)
                # for i in range(8):
                #     print(hex(k[i]))
...
if __name__ == "__main__":
    ...
    if my_piccolo.encrypt(0x0123456789abcdef) == 0x5ec42cea657b89ff:
        print("Test Passed.")